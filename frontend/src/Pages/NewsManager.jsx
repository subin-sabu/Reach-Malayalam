  ...
  useEffect(() => {
    ...
    if (news.length === 0) {
      ...
      const fetchNews = async () => {
        try {
          const queryDescending = query(collection(db, 'news'), orderBy('timestamp', 'desc'));
          // Limit to 30 documents if newsArray is empty and you are directly fetching
          queryDescending.limit(30);
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-evenly', paddingBottom: '1rem', paddingTop: '1rem', marginBottom: '1rem', backgroundColor: '#F8F8F8' }}>
        ...
        <Button onClick={() => navigate('/reportnews')} variant="contained" color="primary">
          Add News
        </Button>
              ...
              {newsList.map((news, index) => (
                <TableRow key={news.id} sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
                  ...
                  {!mobileView && (
                    <>
                      <TableCell>
                        <Button onClick={() => navigate(`/newsmanager/editor/${news.id}`)} style={{ color: 'green' }}>
                          Edit
                        </Button>
                      </TableCell>
                      <TableCell>
                  ...
                  {mobileView && (
                    <TableCell>
                      ...
                      >
                        <MenuItem onClick={() => navigate(`/newsmanager/editor/${news.id}`)} style={{ color: 'green' }}>Edit</MenuItem>
